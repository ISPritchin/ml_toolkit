...
class MondrianForestRegressor(BaseModel):
    ...
    def fit(
        ...
        if self.params is not None:
            self._model = _make_pipeline(_MFRegressor, self.params)
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = _make_pipeline(_MFRegressor, _suggest(trial))
                pipe.fit(Xtr, y_tr)
            ...
            self._model = _make_pipeline(_MFRegressor, self.best_params_)
            self._model.fit(Xtr, y_tr)
...
class MondrianForestClassifier(BaseModel):
    ...
    def fit(
        ...
        if self.params is not None:
            self._model = _make_pipeline(_MFClassifier, self.params)
            self._model.fit(Xtr, y_tr)
            self.best_params_ = self.params
        else:
            if X_valid is None:
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = _make_pipeline(_MFClassifier, _suggest(trial))
                pipe.fit(Xtr, y_tr)
            ...
            self._model = _make_pipeline(_MFClassifier, self.best_params_)
            self._model.fit(Xtr, y_tr)
...
def train_regression(
    ...
    model = MondrianForestRegressor(n_optuna_trials=n_optuna_trials, model_settings=model_settings)
    ...
    logger.info('[MONDRIAN Reg] Final MAE: %.3f', mean_absolute_error(y_valid, valid_pred))
...
def train_classification(
    ...
    model = MondrianForestClassifier(n_optuna_trials=n_optuna_trials, model_settings=model_settings or {})
    model.fit(X_train, y_train, X_valid, y_valid, selected_features, cat_features)
    infer_proba = model.predict_proba(X_inference)
    logger.info('[MONDRIAN Cls] Final PR-AUC: %.3f', average_precision_score(y_valid, model.valid_pred_))
...
def make_predict_fn(model: Any, task: str, selected_features: list[str]) -> Any:
    """Возвращает callable (X → np.ndarray) для permutation importance."""
    _m, _feats = model, selected_features
    if task == 'regression':
        return lambda X: _m.predict(X[_feats])
    return lambda X: _m.predict_proba(X[_feats])[:, 1]
