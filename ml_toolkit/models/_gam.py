...
def train_regression(
    ...
    model = PyGAMRegressor(n_optuna_trials=n_optuna_trials, model_settings=model_settings)
    ...
    logger.info('[PYGAM Reg] Final MAE: %.3f', mean_absolute_error(y_valid, valid_pred))
...
def train_classification(
    ...
    model = PyGAMClassifier(n_optuna_trials=n_optuna_trials, model_settings=model_settings or {})
    model.fit(X_train, y_train, X_valid, y_valid, selected_features, cat_features)
    infer_proba = model.predict_proba(X_inference)
    logger.info('[PYGAM Cls] Final PR-AUC: %.3f', average_precision_score(y_valid, model.valid_pred_))
...
def make_predict_fn(model: Any, task: str, selected_features: list[str]) -> Any:
    ...
    return lambda X: _np.asarray(_m.predict_proba(_prep.transform(X[_nf].to_numpy(dtype=float))))
