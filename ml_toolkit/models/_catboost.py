...
class CatBoostRegressor(BaseModel):
    ...
    .. note::
        ...
        `valid_pred_` — сам `predict()` класса его не знает. Для инференса
        примените `postprocess_fn` к результату `predict()` вручную:
        `postprocess_fn(X_new, model.predict(X_new))`.
