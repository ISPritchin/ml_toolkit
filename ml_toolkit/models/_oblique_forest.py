"""Oblique Random Forest (scikit-tree).

Классический RF использует axis-aligned сплиты (только один признак).
Oblique RF применяет линейные комбинации признаков в каждом узле → ловит диагональные границы.

Требует: pip install scikit-tree
...
try:
    from sktree.ensemble import ObliqueRandomForestClassifier as _OFClassifier
    from sktree.ensemble import ObliqueRandomForestRegressor as _OFRegressor
except ImportError as e:
    raise ImportError(
        'Oblique Random Forest requires scikit-tree package: pip install scikit-tree'
    ) from e
...
def _suggest(trial: optuna.Trial) -> dict:
    return {
        ...
        'feature_combinations': trial.suggest_int('feature_combinations', 1, 5),
        'random_state': 42,
        'n_jobs': -1,
    }
...
class ObliqueForestRegressor(BaseModel):
    """ObliqueRandomForestRegressor (Pipeline + SimpleImputer) с подбором через Optuna.
    ...
    def fit(
        ...
        if self.params is not None:
            self._model = _make_pipeline(_OFRegressor, self.params)
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = _make_pipeline(_OFRegressor, _suggest(trial))
                pipe.fit(Xtr, y_tr)
            ...
            self._model = _make_pipeline(_OFRegressor, self.best_params_)
            self._model.fit(Xtr, y_tr)
...
class ObliqueForestClassifier(BaseModel):
    """ObliqueRandomForestClassifier с подбором через Optuna. Вероятности калибруются изотонической регрессией.

    class_weight='balanced'. params=None → Optuna; params=dict → прямое обучение без тюнинга.
    """
    ...
    def fit(
        ...
        if self.params is not None:
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = _make_pipeline(_OFClassifier, {**_suggest(trial), 'class_weight': 'balanced'})
                pipe.fit(Xtr, y_tr)
            ...
            self._model = _make_pipeline(_OFClassifier, self.best_params_)
            self._model.fit(Xtr, y_tr)
...
def train_regression(
    ...
    model = ObliqueForestRegressor(n_optuna_trials=n_optuna_trials, model_settings=model_settings)
    ...
    logger.info('[OBLIQUE_FOREST Reg] Final MAE: %.3f', mean_absolute_error(y_valid, valid_pred))
...
def train_classification(
    ...
    model = ObliqueForestClassifier(n_optuna_trials=n_optuna_trials, model_settings=model_settings or {})
    model.fit(X_train, y_train, X_valid, y_valid, selected_features, cat_features)
    infer_proba = model.predict_proba(X_inference)
    logger.info('[OBLIQUE_FOREST Cls] Final PR-AUC: %.3f', average_precision_score(y_valid, model.valid_pred_))
