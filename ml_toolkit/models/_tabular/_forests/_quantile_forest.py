...
from ml_toolkit.models._tabular._forests._common import make_impute_pipeline, predict_via_pipeline
...
class QuantileForestRegressor(BaseModel):
    ...
    def fit(
        ...
        if self.params is not None:
            self._model = make_impute_pipeline(_QuantileMedianWrapper, self.params)
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = make_impute_pipeline(_QuantileMedianWrapper, _suggest(trial))
                pipe.fit(Xtr, y_tr)
            ...
            self._model = make_impute_pipeline(_QuantileMedianWrapper, self.best_params_)
            self._model.fit(Xtr, y_tr)
