...
class ObliqueForestRegressor(BaseModel):
    ...
    def fit(
        ...
        if self.params is not None:
            self._model = make_impute_pipeline(_OFRegressor, self.params)
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = make_impute_pipeline(_OFRegressor, _suggest(trial))
                pipe.fit(Xtr, y_tr)
            ...
            self._model = make_impute_pipeline(_OFRegressor, self.best_params_)
            self._model.fit(Xtr, y_tr)
...
class ObliqueForestClassifier(BaseModel):
    ...
    def fit(
        ...
        if self.params is not None:
            ...
            self._model = make_impute_pipeline(_OFClassifier, direct_params)
            self._model.fit(Xtr, y_tr)
            ...
            def objective(trial: optuna.Trial) -> float:
                pipe = make_impute_pipeline(_OFClassifier, {**_suggest(trial), 'class_weight': 'balanced'})
                pipe.fit(Xtr, y_tr)
            ...
            self._model = make_impute_pipeline(_OFClassifier, self.best_params_)
            self._model.fit(Xtr, y_tr)
