...
Каждая модель реализована в отдельном файле `ml_toolkit/models/_tabular/_{family}/_{name}.py` и экспортирует пару классов `XxxRegressor`/`XxxClassifier`, наследующих `BaseModel` (`ml_toolkit/models/_base.py`) — единый контракт `fit(X_train, y_train, X_valid=None, y_valid=None, selected_features=None, cat_features=None)` / `predict(X)` / `predict_proba(X)`. `_tabular/` группирует все табличные supervised-адаптеры по семействам (`_boosting`, `_forests`, `_interpretable`, `_automl`) — общий для семейства код (Optuna-study, препроцессинг-пайплайны и т.п.) вынесен в `_tabular/_{family}/_common.py`. Модели с принципиально другим контрактом `fit`/`predict` (не наследующие `BaseModel`) получают свой сиблинг-раздел на уровне `ml_toolkit/models/`, а не встраиваются в `_tabular/`. Публичный API (`from ml_toolkit.models import XxxRegressor`) не зависит от внутреннего расположения файла — импорт всегда идёт через `ml_toolkit.models`, а не напрямую из подпакета.
...
**Только регрессия.** `InterpretableTreeClassifier` больше не принимает `name='locally_linear_forest'` (raises `ValueError`): Ridge не работает на бинарных таргетах, поэтому классификационная ветка не строила никакой локальной линейной модели — она была тонкой обёрткой вокруг plain `RandomForestClassifier` с урезанным пространством Optuna, дублируя `ml_toolkit.models.RandomForestClassifier` без какой-либо добавленной ценности. Для классификации используйте `RandomForestClassifier` напрямую или `InterpretableTreeClassifier(name='soft_decision_tree')`.
...
1. Определить семейство: если модель — табличная supervised (X_train/y_train, `BaseModel`-контракт), выбрать подходящий подпакет `ml_toolkit/models/_tabular/_{family}/` (`_boosting`, `_forests`, `_interpretable`, `_automl`) либо завести новое семейство там же, если ни одно из существующих не подходит по духу (общий препроцессинг/паттерн тюнинга). Модель с принципиально другим контрактом `fit`/`predict` (не наследующая `BaseModel`) не встраивается в `_tabular/` — для неё заводится свой сиблинг-раздел на уровне `ml_toolkit/models/`.
2. Создать `ml_toolkit/models/_tabular/_{family}/_{name}.py` с классами `XxxRegressor`/`XxxClassifier`, наследующими `BaseModel` (см. «Структура адаптеров» выше). Если в семействе уже есть повторяющийся паттерн (пайплайн препроцессинга, список numeric-признаков и т.п.), переиспользовать/расширить `_tabular/_{family}/_common.py`, а не копировать код.
3. Добавить оба класса в `_LAZY_CLASSES` и `__all__` в `ml_toolkit/models/__init__.py`, указав полный dotted-путь до нового файла.
4. Если модель поддерживает SHAP/permutation importance нестандартным способом, добавить обработку в `ml_toolkit/model_explainer/`.
...
# Locally Linear Forest (только регрессия — см. раздел выше)
