...
def train_regression(
    ...
    """Обучает TabM-регрессор с подбором архитектурных гиперпараметров через Optuna.

    Использует label standardisation (mean/std на train) и early stopping по MAE.
    Возвращает обёртку (final_model, prep, y_stats) для downstream использования.
    ...
    Args:
        ...
        cat_features: Список категориальных признаков.
        model_settings: Настройки: `device`, `n_epochs_per_trial`, `n_epochs_final`,
            `patience`, `batch_size`.
        n_optuna_trials: Число trials Optuna.
        postprocess_fn: Опциональная функция постобработки (X, pred) → pred.

    Returns:
        Кортеж ((final_model, prep, y_stats), train_pred, valid_pred, infer_pred, best_params).
    ...
    patience = int(model_settings.get('patience', 16))
    batch_size = int(model_settings.get('batch_size', 256))
    metric_fn, direction = resolve_metric_fn(
        model_settings, 'reg_metric', REG_METRICS['mae'][0], 'minimize', REG_METRICS,
    )
    logger.info('[TabM Reg] device=%s, epochs_trial=%d, epochs_final=%d', device, n_epochs_trial, n_epochs_final)

    cat_in_sel = [f for f in cat_features if f in selected_features]
    num_features = [f for f in selected_features if f not in cat_in_sel]
    prep = _Preprocessor(num_features, cat_in_sel, n_train=len(X_train)).fit(X_train)

    data_tr = prep.transform(X_train, device)
    data_va = prep.transform(X_valid, device)
    data_in = prep.transform(X_inference, device)

    y_mean = float(y_train.mean())
    y_std = float(y_train.std()) or 1.0
    y_stats = (y_mean, y_std)
    y_tr = torch.as_tensor(((y_train.values - y_mean) / y_std).astype(np.float32), device=device)
    y_va_np = y_valid.values.astype(np.float64)

    n_num = len(num_features)
    card = prep.cat_cardinalities
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        _, score = _run_training(
            tabm, n_num, card, 1, **params,
            task_type='regression',
            data_tr=data_tr, data_va=data_va,
    ...
    bp = study.best_params
    final_model, _ = _run_training(
        tabm, n_num, card, 1,
        k=bp['k'], d_block=bp['d_block'], n_blocks=bp['n_blocks'], dropout=bp['dropout'],
        lr=bp['lr'], weight_decay=bp['weight_decay'],
        task_type='regression',
        data_tr=data_tr, data_va=data_va,
        y_tr=y_tr, y_va_np=y_va_np, y_stats=y_stats,
        n_epochs=n_epochs_final, patience=patience, batch_size=batch_size,
        device=device, X_valid_full=X_valid, postprocess_fn=postprocess_fn,
        metric_fn=metric_fn, direction=direction,
    )
    ...
    def _denorm_predict(data: dict[str, torch.Tensor]) -> np.ndarray:
        raw = _avg_pred(_predict_raw(final_model, data), 'regression') * y_std + y_mean
        return np.nan_to_num(raw, nan=y_mean, posinf=y_mean, neginf=y_mean)

    train_pred = _pp(X_train, _denorm_predict(data_tr))
    valid_pred = _pp(X_valid, _denorm_predict(data_va))
    infer_pred = _pp(X_inference, _denorm_predict(data_in))
    logger.info('[TabM Reg] Final MAE: %.3f', mean_absolute_error(y_valid, valid_pred))
...
def train_classification(
    ...
    """Обучает TabM-классификатор с подбором архитектурных гиперпараметров через Optuna.

    Использует BCE-loss, early stopping по PR-AUC. Вероятности инференса калибруются
    изотонической регрессией на валидационной выборке.

    Args:
        X_train: Обучающая выборка.
        y_train: Бинарная целевая переменная обучающей выборки.
        X_valid: Валидационная выборка.
    ...
    Returns:
        Кортеж ((final_model, prep), train_proba, val_proba, infer_proba_calibrated, best_params).
    ...
    patience = int(cfg.get('patience', 16))
    batch_size = int(cfg.get('batch_size', 256))
    metric_fn, direction = resolve_metric_fn(
        cfg, 'cls_metric', CLS_METRICS['pr_auc'][0], 'maximize', CLS_METRICS,
    )
    logger.info('[TabM Cls] device=%s, epochs_trial=%d, epochs_final=%d', device, n_epochs_trial, n_epochs_final)
    ...
    y_arr = y_train.values if hasattr(y_train, 'values') else np.asarray(y_train)
    y_tr = torch.as_tensor(y_arr.astype(np.float32), device=device)
    y_va_np = (y_valid.values if hasattr(y_valid, 'values') else np.asarray(y_valid)).astype(np.float64)
    ...
    train_proba = np.nan_to_num(_avg_pred(_predict_raw(final_model, data_tr), 'classification'), nan=0.5)
    val_proba = np.nan_to_num(_avg_pred(_predict_raw(final_model, data_va), 'classification'), nan=0.5)
    infer_proba = np.nan_to_num(_avg_pred(_predict_raw(final_model, data_in), 'classification'), nan=0.5)
...
def make_predict_fn(model: Any, task: str, selected_features: list[str]) -> Any:
    """Возвращает callable (X → np.ndarray) с TabM-препроцессингом (imputer + QT) для permutation importance."""
    if task == 'regression':
        _m, _prep, _ys = model
        _dev = next(_m.parameters()).device
        def _fn(X: Any) -> Any:
            d = _prep.transform(X, _dev)
            return _avg_pred(_predict_raw(_m, d), 'regression') * _ys[1] + _ys[0]
    else:
        _m, _prep = model
        _dev = next(_m.parameters()).device
        def _fn(X: Any) -> Any:
            d = _prep.transform(X, _dev)
            return _avg_pred(_predict_raw(_m, d), 'classification')
    return _fn
