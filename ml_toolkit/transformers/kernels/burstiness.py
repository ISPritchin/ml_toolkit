"""Паттерны всплесков активности: burst-периоды, соотношение peak/mean.

Signal:
    Разграничивает равномерно активные ряды от пульсирующих (редкие крупные
    всплески с долгими паузами между ними). Высокий peak_mean при низком burst_count
    указывает на единственный доминирующий выброс.
...
Interpretation:
    peak_mean_w12 > 4, calm_share > 0.5 — редкие, но крупные всплески на фоне долгого затишья.
    peak_mean_w12 ≈ 1.1, calm_share ≈ 0 — равномерный ряд без выраженных всплесков.
    burst_cv_w12 = 0 — строго равномерные по длине вспышки (ритмичный паттерн активности).
    gap_mean_w12 = 2, burst_count = 4 — активность раз в квартал.

Example:
    Ряд (6 мес): [0, 40, 30, 0, 20, 0],  w=6
    mean = 90/6 = 15,  median = (0+20)/2 = 10 (среднее двух центральных сорт. [0,0,0,20,30,40]),  max = 40
    ...
    → burstiness__peak_med_w6   = 40/10 = 4.0
