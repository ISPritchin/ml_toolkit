...
Signal:
    ...
    значение означает, что ряд пережил глубокую просадку: потенциально нестабильный
...
Interpretation:
    ...
    = 0.5 — в какой-то момент ряд потерял 50% от своего локального максимума.
    = 0.9375 — просадка ряда D: с 80 до 5 (классический V-образный пример).
    max_drawdown_w12 > 0.7 при recovery_completeness = 1 — ряд упал и полностью восстановился.
