...
Signal:
    Показывает, растёт или падает нестабильность поведения ряда. Положительное значение —
...
Interpretation:
    ...
    < 0 — ряд стабилизируется: последние месяцы ровнее длинной истории.
...
def compute(values: np.ndarray, position: np.ndarray, params: dict):
    """params: {"pairs": [[3, 12], [6, 12]]} — ключ обязателен, дефолты задаёт пресет."""
    pairs = params['pairs']
