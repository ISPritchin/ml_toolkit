"""Регулярность поступлений: промежутки между активными сериями, CV ритма.

Signal:
    Измеряет предсказуемость ритма клиента: как равномерны интервалы между «включениями»
    и насколько стабильны сами всплески по длине. Низкий CV промежутков при нескольких
    всплесках — клиент с квартальным/полугодовым ритмом (ценно для прогноза).
...
Interpretation:
    gap_cv = 0 — идеально ритмичный клиент (поступления через строго равные интервалы).
    gap_cv > 1 — хаотичный, непредсказуемый ритм.
    cadence_shift = 1 — последние месяцы клиент «замолчал» дольше обычного (риск оттока).
