...
Signal:
    Показывает текущий «простой» ряда (current) и наибольшую паузу за всю историю
    (max). Если current > 0 — ряд не активен прямо сейчас. Высокий max_streak при
    низком current — в прошлом была долгая пауза в активности, сейчас восстанавливается.
...
Interpretation:
    current = 0 — ряд активен в текущем месяце.
    current > 3 — ряд молчит более квартала: длительная пауза в активности.
    max = 6, current = 0 — когда-то выпадал на полгода, сейчас восстановился.
    max > 12 — хроническая нестабильность в прошлом (долгие паузы повторялись).
