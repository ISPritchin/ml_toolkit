...
Signal:
    Показывает, во сколько раз (в %) вырос ряд от момента первой активности
    до текущего месяца. Высокое значение — кратный рост за всю историю; отрицательное
    — ряд сейчас ниже стартового уровня (регресс).
...
Interpretation:
    = +1.0 — значение удвоилось с момента первой активности.
    = 0 — вернулся на стартовый уровень (после роста и падения).
    < -0.5 — потерял более половины от стартового уровня: сильный регресс.
    Для рядов с короткой историей (tenure < 6 мес) значение нестабильно из-за малой базы.
