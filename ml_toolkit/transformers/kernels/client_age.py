...
Signal:
    ...
    сущности одинаковый набор дат, а периоды до реального появления сущности зафилены
    нулями), то position_within_entity считает позицию В СЕТКЕ, а не «сколько сущность
    реально существует». Две сущности с одинаковым числом строк получат ПОБАЙТОВО
...
Interpretation:
    new_client_flag = 1 — это одна из первых 3 строк сущности в наблюдаемых данных;
        НЕ обязательно означает «сущность недавно стала активной» — если ряд зафилен
    ...
    months_since_start_norm — насколько далеко эта строка от начала наблюдаемого окна
        сущности, а не от начала реальной активности сущности.
    ...
    Для «сколько времени сущность реально активна с первого ненулевого наблюдения» —
    используйте `tenure` (tenure_months / first_active_flag), а не этот модуль.

Example:
    Сущность A (активна с первой строки): values = [35, 20, 12, ...], pos = 0..N-1
    Сущность B (те же даты, но появилась в середине; до этого — нули):
        values = [0, 0, ..., 0, 34, 25, ...], pos = 0..N-1

    На одинаковой позиции pos=15 (та же календарная дата) обе сущности получат
    ОДИНАКОВЫЙ months_since_start_norm = 15/(15+12) = 0.556 — несмотря на то что A
    активна уже 16 месяцев, а B — только 4 (см. tenure_months для корректного различия).
