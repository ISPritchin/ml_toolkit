...
Signal:
    Показывает, ускоряется ли динамика ряда (рост темпа роста) или тормозит
...
Interpretation:
    > 0 — тренд ускоряется (приросты нарастают), ряд в фазе разгона.
...
FEATURE = 'accel'
FILL_NAN: dict[str | None, float] = {None: FILL_NAN_UNBOUNDED}  # вторая разность в единицах колонки, без универсальной границы
