...
# Единый сентинел для FILL_NAN у любой величины, для которой нет математического
# доказательства, что конкретное "маленькое" число (-1.0/-2.0) недостижимо —
# это касается как сырых по масштабу колонки величин (mean/sum/slope/diff и т.п.,
# знак и порядок которых не ограничены, если не считать данные заведомо
# неотрицательными), так и выходов safe_ratio, где числитель/знаменатель не
# доказаны структурно однозначного знака (клампирован лишь диапазон [-1e6, 1e6],
# а не что-то более узкое). Единственное, что действительно доказано формально
# недостижимо для строго ограниченного диапазона (Pearson r в [-1,1], доля в
# [0,1], sqrt(var) >= 0 и т.п.) — по-прежнему может использовать -1.0/-2.0
# напрямую; для всего остального — этот единый сентинел, вне зависимости от
# формы/знака ряда. -1e100 сюда не годится: все фичи приводятся к float32 перед
# записью в parquet (max ~3.4e38), и такая величина переполнилась бы в -inf.
# -1e30 безопасно укладывается в float32 и при этом на много порядков вне
# реалистичного диапазона бизнес-данных.
FILL_NAN_UNBOUNDED = -1e30

# Симметричный положительный вариант — для величин, где "нет значения" должно
# читаться как заведомо большое, а не заведомо маленькое (например, у
# отсутствующего минимума скользящего окна: заполнение через FILL_NAN_UNBOUNDED
# выглядело бы как правдоподобно низкий реальный минимум, а не как явный сентинел).
FILL_NAN_UNBOUNDED_HIGH = -FILL_NAN_UNBOUNDED
