...
**92 трансформера · 328 выходных колонок на product-колонку** (число приведено для референсной конфигурации со всеми 92 трансформерами и параметрами, показанными в секциях `Preset entry` ниже — no library-shipped "full" preset with this configuration exists; preset обязателен и всегда явный, автоматического дефолта нет, см. CLAUDE.md → Preset system. Другой набор трансформеров/параметров даёт другое число — см. таблицу в конце документа).
...
Документ сгенерирован по актуальным докстрингам кернелов (секции Signal/Formula/Outputs/Preset entry/Interpretation/Example, см. правило в `CLAUDE.md`) — формулы и примеры здесь идентичны тем, что лежат в коде.
...
- **Неотрицательность.** `product_values` ожидаются неотрицательными. Кернелы не падают на отрицательных значениях, но лог-признаки (`log1p(|v|)`) и ratio-признаки теряют знак/интерпретацию.
...
**Сигнал:** куда идёт ряд и насколько устойчиво.
...
**Сигнал:** насколько непредсказуем ряд.
...
- `extreme_share__extreme_w12 > 0.3` — более 30% месяцев выходят за 1.5σ: нестабильный ряд.
...
**Сигнал:** как давно ряд существует, как долго молчал, насколько регулярен.

| Колонка | Формула |
|---------|---------|
| `tenure__tenure_months`, `__first_active_flag` | Месяцев с первого ненулевого значения (running); флаг самой строки первой активации |
...
**Интерпретация:**
- `active_months_w12 = 12` — ряд активен каждый месяц; `3–5` — редкие, но крупные ненулевые наблюдения.
- `inactive_streak__max = 6, current = 0` — когда-то выпадал на полгода, сейчас восстановился.
- `activity_rate > 0.9` — практически без пропусков; `< 0.3` — редкая активность.
...
- `zscore_w12 > 2` — аномально высокий месяц (разовый выброс/сезонный пик).
- `mean_median_gap > 0.3` — сильная правосторонняя асимметрия (нули + редкие всплески).
...
- `trough_to_current > 2` — удвоение значения от минимума окна (сильное восстановление).
...
- `entropy ≈ 1.0` — значения равномерно распределены по месяцам; `≈ 0` — весь объём в одном месяце.
- `gini > 0.6` — экстремальная концентрация (1-2 месяца дают почти весь объём).
- `cusum_pos_w12 >> |cusum_neg_w12|` — распределение смещено вправо (редкие большие всплески).
...
**Интерпретация:**
- `lag12 ≈ +1` — сильная годовая сезонность; `lag6 ≈ +1` — полугодовая периодичность.
...
**Сигнал:** темп и устойчивость роста в лог-шкале — устойчивы к экспоненциальному распределению значений.
...
**Пример (`geometric_return`, ряд `[10,20,40,80]`, w=4 — удвоение каждый месяц):** `mean(log_diff) ≈ 0.665` → `geometric_return__w4 = exp(0.665)-1 = 0.945` (≈+95%/мес).
...
**Интерпретация:**
- `kurt > 3` — тяжёлые хвосты (редкие экстремальные месяцы); `< -1` — плоское, равномерное распределение.
...
## 14. `burstiness` — редкие всплески vs равномерный поток
...
**Интерпретация:** `peak_mean > 4, calm_share > 0.5` — редкие, но крупные всплески на фоне долгого затишья; `peak_mean ≈ 1.1, calm_share ≈ 0` — равномерный ряд без всплесков.

**Пример (ряд `[0,40,30,0,20,0]`, w=6):** `mean=15, median=10, max=40`; серии `[40,30]` и `[20]` → `burst_count=2, burst_dur=1.5, gap_mean=1.5, calm_share=0.5`; `peak_mean_w6=40/15=2.667`, `peak_med_w6=40/10=4.0`.

---

## 15. `flow_regularity` — регулярность активности
...
**Интерпретация:** `gap_cv = 0` — идеально ритмичный ряд; `cadence_shift = 1` — ряд «замолчал» дольше обычного, чем раньше.
...
**Сигнал:** насколько и как быстро ряд восстановился от минимума окна.
...
**Сигнал:** много равных небольших приростов («органичный» рост) vs один крупный скачок («скачкообразный»).
...
| `growth_quality__neg_sum_share_w12` | Доля суммарных потерь к среднему уровню ряда |

**Интерпретация:** `organic ≈ 0.91` — рост равномерно распределён по месяцам; `organic ≈ 0` — весь рост от одного крупного скачка.
...
**Интерпретация:** `lag12_ratio = +1.0` — значение удвоилось год к году; `yoy_accel > 0` — YoY-рост ускоряется.
...
**Сигнал:** резкий переход с одного уровня значений на другой (перебор всех точек разбиения окна).
...
**Интерпретация:** `magnitude=2.33, flag=1` — явная структурная смена уровня ряда; `current_regime_len=0` — сдвиг обнаружен прямо сейчас.
...
| `trend_consistency__noise_signal_w6/12` | `safe_ratio(RMSE остатков, \|slope\|·w)`; 0 и при почти нулевом наклоне, не только при чистом тренде |
...
**Сигнал:** стабильно ли ряд удерживает «хорошие»/«плохие» позиции в своей истории.
...
## 25. `lifecycle_phase` — жизненный цикл ряда
...
**Сигнал:** насколько «типичен» текущий месяц для ряда.
...
**Интерпретация:** `predictability ≈ 0.99` — почти идеально предсказуемый ряд; `≈ 0.40` — высокая нестабильность.
...
**Сигнал:** доля объёма, приходящаяся на топ-1/топ-3 месяца; индекс Херфиндаля.
...
**Интерпретация:** `max_zero_run=6` — ряд полгода подряд оставался нулевым.
...
## 32. `c3` — time-reversal asymmetry

**Файл:** `c3.py`

**Сигнал:** момент третьего порядка `E[v[t]·v[t+lag]·v[t+2·lag]]`, нормированный на `std_w³` — детектор нелинейной, необратимой во времени динамики, которую линейная автокорреляция (`autocorr`, `corr_with_time`) не видит.

| Колонка | Формула |
|---------|---------|
| `c3__lag1_w6`, `__lag1_w12` | `safe_ratio(mean(v[i]·v[i+L]·v[i+2L]), std_w³)` |

**Интерпретация:** `≈ 0` — динамика согласуется с линейным симметричным во времени процессом; сильно `≠ 0` (знак малоинформативен) — обнаружена нелинейная time-irreversible структура.

**Пример (ряд `[1,2,4,2,1]`, lag=1, w=5):** тройки `(1,2,4)→8, (2,4,2)→16, (4,2,1)→8` → `c3_raw=10.667`; `mean_w=2.0, std_w=1.095` → `c3__lag1_w5 = 10.667/1.095³ = 8.11`.

---

## 33. `permutation_entropy` — энтропия ordinal-паттернов

**Файл:** `permutation_entropy.py`

**Сигнал:** энтропия распределения ordinal-паттернов (порядка соседних троек значений), а не уровня — `entropy.py` меряет равномерность ОБЪЁМА, этот трансформер меряет предсказуемость ПОРЯДКА взлётов/падений. Embedding dimension `m=3`, delay `tau=1` зафиксированы (при окнах 6/12/24 `m=4` не даёт статистически надёжной оценки).

| Колонка | Формула |
|---------|---------|
| `permutation_entropy__w6`, `__w12`, `__w24` | `-Σ(p_k·ln p_k)/ln(6)`, `p_k` — доля троек с ordinal-паттерном `k` |

**Интерпретация:** `≈ 1.0` — порядок взлётов/падений максимально непредсказуем; `≈ 0.0` — динамика строго регулярна (например, монотонный тренд на всё окно).

**Пример (ряд `[10,20,30,40,50,60]`, w=6):** все тройки возрастающие (один и тот же паттерн) → `permutation_entropy__w6 = 0.0`.

---

## 34. `change_quantiles` — условная волатильность внутри коридора значений

**Файл:** `change_quantiles.py`

**Сигнал:** волатильность приращений, посчитанная ТОЛЬКО между точками внутри коридора `[ql, qh]` (квантили окна) — в отличие от `rolling_std`/`rolling_cv` (безусловные по всему окну), отделяет шум в типичном диапазоне от волатильности, вызванной редкими экстремумами.

| Колонка | Формула |
|---------|---------|
| `change_quantiles__mean_w6/12` | `mean(\|v[i]-v[i-1]\|)` по парам, обе точки которых в `[lo_w, hi_w]` |
| `change_quantiles__std_w6/12` | `std` того же множества приращений |

**Интерпретация:** высокий `mean` при низком `rolling_std` — шум сосредоточен в теле распределения, а не в хвостах (выбросы уже исключены коридором).

**Пример (ряд `[5,20,22,21,23,5]`, w=6, ql=0.2, qh=0.8):** коридор `[5,22]`; квалифицирующие пары `(5,20)→15, (20,22)→2, (22,21)→1` → `mean_w6=6.0, std_w6=6.377`.

---

## 35. `energy_ratio_by_chunks` — доля энергии по третям окна

**Файл:** `energy_ratio_by_chunks.py`

**Сигнал:** доля суммарной энергии (`Σv²`) окна, приходящаяся на первую/среднюю/последнюю треть (метод третей, как в `nonlinearity`). В отличие от `value_clustering` (доли по рангу значений) — здесь первична позиция во времени, а квадрат снимает знак и сильно взвешивает крупные выбросы.

| Колонка | Формула |
|---------|---------|
| `energy_ratio_by_chunks__first_w6/12`, `__mid_w6/12`, `__last_w6/12` | `safe_ratio(Σv² трети, Σv² всего окна)` |

**Интерпретация:** `last_share >> first_share` — энергия сконцентрирована в конце окна (недавний всплеск доминирует).

**Пример (ряд `[5,5,10,10,30,30]`, w=6):** `e1=50, e2=200, e3=1800`, `total=2050` → `first_w6=0.024, mid_w6=0.098, last_w6=0.878`.

---

## 36. `index_mass_quantile` — тайминг накопления массы

**Файл:** `index_mass_quantile.py`

**Сигнал:** доля длины окна, за которую кумулятивная сумма достигает 25%/50%/75% полного объёма — устойчивая (по всей массе, а не по единичному пику) альтернатива `lifecycle_phase__peak_age_share`.

| Колонка | Формула |
|---------|---------|
| `index_mass_quantile__q25_w12/24`, `__q50_w12/24`, `__q75_w12/24` | `idx_q / (ws-1)`, где `idx_q` — первая позиция, на которой `running_sum >= q·total_w` |

**Интерпретация:** `q50 < 0.3` — активность сфронтирована в начало окна; `q50 > 0.7` — разгон к концу окна.

**Пример (ряд `[0,0,10,10,10,10]`, w=6):** `total=40`, `running=[0,0,10,20,30,40]` → `q25_w6=0.4, q50_w6=0.6, q75_w6=0.8`.

---

## 37. `agg_autocorrelation` — сводка ACF по диапазону лагов

**Файл:** `agg_autocorrelation.py`

**Сигнал:** в отличие от `autocorr` (конкретные лаги 1/2/3/6/12), считает windowed Pearson для ВСЕХ лагов `1..max_lag` и сводит в mean/|mean|/std — ловит память на лагах, не входящих в фиксированный список, и отличает гладкое затухание ACF от шумного скачущего.

| Колонка | Формула |
|---------|---------|
| `agg_autocorrelation__mean_w24` | `mean(r_lag)` по валидным лагам `1..max_lag` |
| `agg_autocorrelation__abs_mean_w24` | `mean(\|r_lag\|)` |
| `agg_autocorrelation__std_w24` | `std(r_lag)` |

**Интерпретация:** высокий `abs_mean` при `mean ≈ 0` — память есть, но знак чередуется по лагам (осцилляция); высокий `std` — ACF ненадёжно скачет от лага к лагу.

**Пример (ряд `[10,20,30,40,50,60]`, w=6, max_lag=2):** `r_lag1=r_lag2=1.0` (строго линейный рост) → `mean_w6=1.0, abs_mean_w6=1.0, std_w6=0.0`.

---

## 38. `dfa` — детрендированный флуктуационный анализ (фрактальное масштабирование)

**Файл:** `dfa.py`

**Сигнал:** экспонента фрактального масштабирования, устойчивая к тренду — сначала убирает локальный линейный тренд внутри блоков окна, затем смотрит, как растёт остаточная флуктуация с размером блока. Два масштаба (small=ws//4, large=ws//2) — компромисс для коротких окон этой библиотеки (классический DFA использует 10+ масштабов на рядах из сотен-тысяч точек).

| Колонка | Формула |
|---------|---------|
| `dfa__alpha_w12`, `__alpha_w24` | `(ln F(n_large) - ln F(n_small)) / (ln n_large - ln n_small)`, F(n) — RMS OLS-остатков по блокам размера n профиля (кумулятивной суммы центрированного ряда) |

**Интерпретация:** `≈0.5` — белый шум без памяти; `≈1.0` — устойчивая долгосрочная память (persistent); `>1.2` — нестационарность/тренд; `<0.4` — antipersistent (частые развороты).

**Пример (ряд `[10,12,11,13,12,14,13,15,14,16,15,17]`, w=12):** тренд +0.5/мес с шумом → `dfa__alpha_w12 ≈ 0.949`. Для сравнения на других рядах: чистый линейный тренд → `≈2.40`, чередующийся ряд без тренда → `≈0.02`, константа → `0`.

---

## 39. `transition_matrix` — марковская структура режимов

**Файл:** `transition_matrix.py`

**Сигнал:** квантование окна в 3 терцильных состояния (низкое/среднее/высокое) + матрица частот переходов между ними — другой взгляд на «смену режима», чем один оптимальный разрыв в `regime_change`.

| Колонка | Формула |
|---------|---------|
| `transition_matrix__stickiness_w12/24` | `(count[0,0]+count[1,1]+count[2,2]) / n_pairs` — доля переходов «в то же состояние» |
| `transition_matrix__trans_entropy_w12/24` | Нормированная энтропия совместного распределения пар состояний `/ ln(9)` |

**Интерпретация:** `stickiness ≈ 1.0` — устойчивый режим, редкие переключения; `≈ 1/3` — переключения не реже случайных. Высокий `trans_entropy` — структура переходов близка к случайной по всем 9 парам.

**Пример (ряд `[10,10,50,50,90,90]`, w=6):** терцильные состояния `[0,0,1,1,2,2]` → `stickiness_w6=0.6, trans_entropy_w6=0.733`.

---

## 40. `acf_characteristic_scale` — характерный масштаб памяти ряда

**Файл:** `acf_characteristic_scale.py`

**Сигнал:** вместо фиксированных лагов (`autocorr`) или их сводки (`agg_autocorrelation`) ищет САМИ лаги, на которых память ряда характерно меняется: где ACF впервые опускается ниже 1/e, и где у неё первый локальный минимум.

| Колонка | Формула |
|---------|---------|
| `acf_characteristic_scale__f1ecac_w24` | Первый `lag`, где `r_lag <= 1/e`; если не найден в `max_lag` — censored до `max_lag` |
| `acf_characteristic_scale__first_min_ac_w24` | Первый локальный минимум ACF; то же censored-соглашение |

**Интерпретация:** `f1ecac=1` — память угасает мгновенно; `= max_lag` (censored) — память держится дольше всего измеренного диапазона, стоит увеличить `max_lag`. `0` (не censored) означает буквально «недостаточно истории» — единственный случай, где ноль не censored-граница.

**Пример (ряд `[10,9,7,4,3,4,7,9]`, w=8, max_lag=4):** `r_1..r_4 = 0.643,-0.283,-0.921,-0.984` → `f1ecac_w8=2.0` (первое пересечение 1/e), `first_min_ac_w8=4.0` (ACF монотонно убывает во всём диапазоне — censored).

---

## 41. `automutual_info` — нелинейное обобщение автокорреляции

**Файл:** `automutual_info.py`

**Сигнал:** взаимная информация между терцильным состоянием `lag` назад и текущим (та же квантовка, что в `transition_matrix`) — ловит любую статистическую зависимость, не только линейную, которую видит Пирсон в `autocorr`/`agg_autocorrelation`.

| Колонка | Формула |
|---------|---------|
| `automutual_info__lag1_w12`, `__lag1_w24` | `Σ(p_ab·ln(p_ab/(p_a·p_b))) / ln(3)` по совместному распределению терцильных состояний `(state[i-lag], state[i])` |

**Интерпретация:** заметно `> 0` при `autocorr__lag1 ≈ 0` — зависимость есть, но нелинейная (сверьте с `c3` для направления асимметрии).

**Пример (ряд `[10,10,50,50,90,90]`, lag=1, w=6):** состояния `[0,0,1,1,2,2]` → `MI=0.500` нат → `automutual_info__lag1_w6 = 0.500/ln(3) = 0.455`.

---

## 42. `auto_period` — автоматически обнаруженный доминирующий период

**Файл:** `auto_period.py`

**Сигнал:** вместо предположения о периоде (`seasonal_autocorr` проверяет только лаги 6 и 12) перебирает лаги в `[min_lag..max_lag]` и берёт тот, где windowed-корреляция максимальна — «бедный, но честный» аналог спектрального пика без FFT (`numba.njit` не поддерживает `np.fft`).

| Колонка | Формула |
|---------|---------|
| `auto_period__period_w24` | `lag*` с максимальным `r_lag` (при равенстве — наименьший лаг, чтобы гармоники не маскировали период) |
| `auto_period__strength_w24` | `r_lag*` — сила корреляции на найденном периоде |

**Интерпретация:** `strength < 0.3` при любом `period` — периодичности как таковой нет, не воспринимайте `period` как факт. `period` около границы диапазона — реальный период, вероятно, за пределами `[min_lag, max_lag]`.

**Пример (ряд `[10,30,10,30,...]`, w=12, min_lag=2, max_lag=4):** `r_lag2=1.0` (истинный период), `r_lag3=-1.0` (противофаза), `r_lag4=1.0` (гармоника, но найдена позже) → `auto_period__period_w12=2.0, strength_w12=1.0`.

---

## Итоговая таблица

Число выходных колонок приведено для референсной конфигурации (все 92 трансформера, параметры как в секциях `Preset entry` выше) — это не пресет, который реально поставляется библиотекой (см. CLAUDE.md → Preset system: сейчас в `presets/` шаблонов только два — `minimum.yaml`, только `slope`, и `minimum_with_segments.yaml`, демонстрирующий синтаксис `segments:`). Любой реальный пресет с другим набором трансформеров/параметров даёт другое число.
...
| Time-reversal asymmetry | `c3` | 2 |
| Ordinal-энтропия | `permutation_entropy` | 3 |
| Условная волатильность | `change_quantiles` | 4 |
| Энергия по третям окна | `energy_ratio_by_chunks` | 6 |
| Тайминг накопления массы | `index_mass_quantile` | 6 |
| Сводка ACF по лагам | `agg_autocorrelation` | 3 |
| DFA (фрактальное масштабирование) | `dfa` | 2 |
| Марковские состояния | `transition_matrix` | 4 |
| Характерный масштаб памяти | `acf_characteristic_scale` | 2 |
| Automutual information | `automutual_info` | 2 |
| Автообнаружение периода | `auto_period` | 2 |
| **Итого (92 трансформера)** | | **328** |
...
Большинство признаков безразмерны или нормированы (ratio/CV/gini/entropy и т.п.) — не требуют масштабирования перед линейными моделями. Исключения — признаки в единицах исходной product-колонки: `rolling_sum`, `rolling_std`, `rolling_min_max__min/max`, `cusum__pos/neg`, `total_variation` (без `norm_` версии), `max_abs_jump`, `volatility_of_diff`, `time_weighted_momentum`, `change_quantiles__mean/std` — если сравниваемые ряды сильно различаются по масштабу значений, эти признаки стоит логарифмировать/нормировать отдельно. `c3` уже нормирован на `std³` (безразмерен), несмотря на то что считается через произведение трёх сырых значений.

### Пресеты

Параметры (`windows`/`pairs`/`lags`/`half_windows`/`alphas` и т.д.) вынесены из кода в `ml_toolkit/transformers/presets/*.yaml`. Библиотека не поставляет «полный» пресет со всеми 81 трансформерами — числа в таблице выше относятся к референсной конфигурации, собранной из параметров, документированных в секции `Preset entry` каждого кернела, а не к одному файлу. Preset всегда обязателен и явный (см. раздел «Preset system» в `CLAUDE.md`); добавляйте собственные именованные пресеты под конкретные задачи по мере необходимости.
