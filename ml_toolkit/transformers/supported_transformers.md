...
**81 трансформер · 292 выходные колонки на product-колонку** (с дефолтным пресетом
`monthly.yaml`; другой пресет даёт другое число — см. таблицу в конце документа).
...
Число выходных колонок — с дефолтным пресетом `ml_toolkit/transformers/presets/monthly.yaml`
(другой пресет из `presets/` даёт другое число: набор `windows`/`pairs`/`lags` там иной).
