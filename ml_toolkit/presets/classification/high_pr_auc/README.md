...
21 специализированный пресет для бинарной классификации при доле позитивов < 5%
(типично < 1%). Все нацелены на максимизацию **PR-AUC**, а не ROC-AUC.
...
│   ├─ Со стэкингом OOB → SubsampleStacking
...
## Кастомные лоссы (FocalLoss / TverskyLoss / PolyLoss / AsymmetricLoss / LDAM)

`FocalLossClassifier`, `TverskyLossClassifier`, `PolyLossClassifier`, `AsymmetricLossClassifier` и `LDAMClassifier` — CatBoost с одним кастомным Python-лоссом из `ml_toolkit.losses` вместо Logloss. Все пять — тонкие подклассы общего движка `_custom_loss_base.py` (`_CustomLossClassifierBase` + `_LossSpec`): fit/Optuna-тюнинг/predict_proba реализованы один раз, каждый класс лишь объявляет свой `_LossSpec` (класс лосса + границы Optuna-поиска его параметров) и именованные kwargs конструктора. Добавить новый кастомный лосс — значит добавить такой же тонкий файл, а не копировать fit/tune заново.

Общее для всех пяти:
- `base_params: dict | None` — параметры CatBoost (без `loss_function`); `None` → дефолтные.
- `n_optuna_trials: int = 0` — если > 0, Optuna ищет **и** параметры лосса, **и** гиперпараметры CatBoost совместно (архитектурный поиск идентичен во всех пяти); первый триал — значения из `__init__`, чтобы явно заданные не терялись молча.
...
`LDAMClassifier` — особый случай: его лосс (`LDAMLoss`) нуждается в статистике датасета (n_pos/n_neg) и в фактическом числе итераций модели, которых нет в момент `__init__`. Для этого `_CustomLossClassifierBase._make_loss(loss_params, *, tr_pool, arch_params)` принимает пул и архитектурные параметры — большинству лоссов они не нужны (дефолтная реализация их игнорирует), `LDAMClassifier` — единственный, кто эту сигнатуру переопределяет.
...
| `reweight_epoch_frac` | 0.8 | Доля итераций до включения DRW-переweighting (рекомендуется 0.5–0.95) |
