"""Тесты для LightGBMRegressor/LightGBMClassifier (ml_toolkit/models/_tabular/_boosting/_lightgbm.py)."""
...
class TestLightGBMUndersampleMajority:
    ...
    def test_default_false_trains_on_full_data(self, classification_data, caplog):
        import logging
        X_train, y_train, X_valid, y_valid = classification_data
        with caplog.at_level(logging.INFO, logger='ml_toolkit.models._tabular._boosting._lightgbm'):
            model = LightGBMClassifier(n_optuna_trials=2)
            model.fit(X_train, y_train, X_valid, y_valid)
        assert_valid_proba(model, X_valid)
