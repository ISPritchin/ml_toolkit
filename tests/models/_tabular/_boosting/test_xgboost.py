"""Тесты для XGBoostRegressor/XGBoostClassifier (ml_toolkit/models/_tabular/_boosting/_xgboost.py).
...
class TestXGBoostUndersampleMajority:
    def test_default_false_trains_on_full_data(self, classification_data, caplog):
        import logging
        X_train, y_train, X_valid, y_valid = classification_data
        with caplog.at_level(logging.INFO, logger='ml_toolkit.models._tabular._boosting._xgboost'):
            model = XGBoostClassifier(n_optuna_trials=2)
            model.fit(X_train, y_train, X_valid, y_valid)
        assert_valid_proba(model, X_valid)
